...
const SubAdmin = () => {
  ...
  const handleReportClick = async (reportType = "daily") => {
    try {
      ...
      setError(null);
      const endpoint =
        reportType === "weekly" ? "weekly-reports" : "daily-reports";
      ...
      const isSpecialDomain =
        ...
        mentorDomain === "xyz" ||
        mentorDomain === "Admin" ||
        mentorDomain === "Super Admin" ||
        mentorDomain === "Management" ||
        (adminRole && adminRole.toLowerCase().includes("super"));

      const domainReports = isSpecialDomain
        ? data
        : data.filter((r) => (r.domain || r.Domain) === mentorDomain);
            ...
            setReportsData(
              isSpecialDomain
                ...
                : updatedReports.filter(
                    (r) => (r.domain || r.Domain) === mentorDomain,
                  ),
            );
          }
        } catch (err) {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (uRes.ok && lRes.ok) {
        ...
        const filteredUsers = isSpecialDomain
          ? data
          : data.filter(
              (u) =>
                (u.domain || u.Domain || "").toLowerCase() ===
                mentorDomain.toLowerCase(),
            );
    ...
    try {
      ...
      if (uRes.ok && lRes.ok) {
        ...
        const isSpecialDomain =
          ...
          mentorDomain === "Super Admin" ||
          mentorDomain === "Management";
        const domainUsers = isSpecialDomain
          ? allUsers
          : allUsers.filter((u) => {
        ...
        if (mentorDomain && mentorDomain !== "Domain Mentor") {
          ...
          const activityPercentage =
            domainUsers.length > 0
              ? Math.round((activeCount / domainUsers.length) * 100)
              : 0;
  ...
  const handleLogout = async () => {
    try {
      ...
      // Even if there's an error, log out the user from the frontend
      localStorage.removeItem("currentUser");
      localStorage.removeItem("token");
      navigate("/login");
    }
  ...
  useEffect(() => {
    ...
    if (storedUser && usersList.length > 0) {
      ...
      const foundUser = usersList.find(
        (u) => u.username?.trim() === parsedUser.username?.trim(),
      );
      if (foundUser) {
        setCurrentUser((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    ...
    if (path.endsWith("/users")) {
      ...
      path.endsWith("/assigned-tasks") ||
      path.endsWith("/task-assigning")
    ) {
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          ...
          <SidebarItem
            ...
            onClick={() => handleSidebarNav("Send Credentials")}
          />
          <SidebarItem
            icon={<TaskIcon />}
            label="Task Assigning"
            active={
              currentView === "task-assigning" || currentView === "assign-task"
            }
          ...
          <SidebarItem
            icon={<FileTextIcon />}
            label="Daily Reports"
            active={currentView === "daily-reports"}
            onClick={() => handleSidebarNav("Daily Reports")}
          />
          <SidebarItem
            icon={<FileTextIcon />}
            label="Weekly Reports"
            active={currentView === "weekly-reports"}
            onClick={() => handleSidebarNav("Weekly Reports")}
          />
        </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 bg-amber-500 text-white flex items-center justify-between px-4 sm:px-6 lg:px-8 shadow-sm">
          <div className="flex items-center flex-1">
            <button
              ...
              className="md:hidden mr-4 p-2 text-white/80 hover:bg-white/10 rounded-lg"
            >
              <MenuIcon />
            </button>
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:text-slate-800">
          ...
          {currentView === "dashboard" ? (
                      ...
                      <div className="flex gap-8 items-center flex-1">
                        <div className="flex-1">
                          <div className="flex items-baseline justify-between mb-1">
                            ...
                            <span className="text-2xl font-bold text-white/40 ml-4">
                              {usersList.length || "0"}
                            </span>
                          </div>
                          <p className="text-blue-100 opacity-80 text-xs">
                            Members in System
                          </p>
                        </div>
                        ...
                        <div className="flex-1">
                          <div className="flex items-baseline justify-between mb-1">
                            ...
                            <span className="text-2xl font-bold text-white/40 ml-4">
                              {usersList.filter(
                                (u) => u.domain === currentUser.domain,
                              ).length || "0"}
                            </span>
                          </div>
                          <p className="text-blue-100 opacity-80 text-xs line-clamp-1">
                            Users in {currentUser.domain}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {(activities.length > 0 ? activities : [])
                        .slice(0, 5)
                        .map((log) => {
                          const loginDate = log.login_time
                          ...
                          return (
                            <ActivityRow
                              key={log.id}                              
                              timestamp={log.login_time || log.logout_time || log.timestamp}
                      ...
                      {activities.length === 0 && (
                        <tr>
                          <td
                            ...
                            className="px-6 py-8 text-center text-slate-400"
                          >
                            No recent activities found in your domain (
                            {currentUser.domain}).
                          </td>
                        </tr>
                      )}
            ...
            <div className="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-linear-to-r from-blue-50 to-white">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1 ml-10">
                    Viewing all users from the central system
                  </p>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {usersList.filter((u) => u.domain === currentUser.domain)
                      .length === 0 ? (
                      <tr>
                        <td
                          colSpan="5"
                      ...
                      usersList
                        .filter((u) => u.domain === currentUser.domain)
                        .map((user) => (
                          <tr
                            key={user.id}
                          ...
                          >
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 font-bold text-xs border">
                            ...
                            {/* Animated Role Badge */}
                            <td className="px-6 py-4">
                              <span className="px-3 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-700 border border-purple-200 animate-pulse">
                                {user.role}
                              </span>
                            </td>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-slate-500 mb-8">
                  This module is ready for credential management and
                  communication tools.
                </p>
                <div className="p-6 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50">
                  <p className="text-sm text-slate-400 font-medium italic">
                    [ Developer Note: Custom data and functionality for sending
                    user credentials can be implemented here. ]
                  </p>
                </div>
                <button
                  onClick={() => navigate("/mentor")}
            ...
            <div className="p-6 bg-slate-50">
              <form
                ...
                className="max-w-4xl mx-auto bg-white p-6 rounded-xl shadow-sm border border-slate-200"
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const selectedUser = usersList.find(
                          (u) => (u.custom_id || String(u.id)) === selectedId,
                        );
                    ...
                    >
                      ...
                      {usersList.map((u) => (
                        <option key={u.id} value={u.custom_id || u.id}>
                          {u.custom_id || u.id}
                        </option>
                      ))}
                    </select>
              ...
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-amber-50/30">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1 ml-8">
                    Assign and monitor domain-specific tasks
                  </p>
                </div>
                <button
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-right">
                            ...
                            <button
                              ...
                              className={`mr-3 px-3 py-1 rounded-md text-xs font-bold transition-colors ${
                                task.isChecked
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-amber-50/30">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1 ml-8">
                    System-wide{" "}
                    {currentView === "weekly-reports" ? "weekly" : "daily"}{" "}
                    progress tracking
                  </p>
                </div>
                  ...
                  <thead className="bg-slate-50 text-slate-500 uppercase text-xs font-semibold">
                    <tr>
                      ...
                      <th className="px-6 py-4">
                        {currentView === "weekly-reports" ? "Week" : "Date"}
                      </th>
...
const SidebarItem = ({
  ...
  const baseClasses =
    "flex items-center justify-between px-4 py-3 rounded-lg transition-all duration-200 cursor-pointer font-medium";
...
const UserRow = ({
  ...
  const logDate = formatIndianDateOnly(loginTime || logoutTime);
  const displayLoginTime = formatIndianTimeOnly(loginTime);
  const displayLogoutTime = formatIndianTimeOnly(logoutTime);
  ...
  return (
    <tr className="hover:bg-slate-50 transition-colors">
      ...
      <td className="px-6 py-4 text-slate-500 text-xs">{displayLoginTime}</td>
      <td className="px-6 py-4 text-slate-500 text-xs">{displayLogoutTime}</td>
      <td className="px-6 py-4">
...
const ActivityRow = ({ timestamp, activity, status }) => (
  <tr className="hover:bg-slate-50 transition-colors">
    <td className="px-6 py-4 font-medium text-slate-700">
      {formatIndianDateTime(timestamp)}
    </td>
    ...
    <td className="px-6 py-4">
      <span
        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold border whitespace-nowrap ${
          status === "Online"
